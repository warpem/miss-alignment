...
model_checkpoint = pathlib.Path(
    "/home/marten/data/datasets/shrec_v2/run1/models/lightning_logs/"
    "version_2/checkpoints/epoch=25--step=1612--loss_epoch=0.29.ckpt"
)
...
for gt, xcorr in zip(gt_files, xcorr_files):
    ...
    grid_search = torch.linspace(-20, 20, 21)
    scores = list()
    for y_grid in grid_search:
        ...
        scores += [score.item()]
        abs_diff = torch.abs(ground_truth_alignment - interpolated_alignment)
        ax.plot(
            abs_diff[:, 1],
            # color="tab:blue",
    ...
    ax.set_ylabel("abs. diff. to ground truth")
    ax.legend()
    plt.show()
