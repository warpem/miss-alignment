...
def plot_volume_slices(
    volume_dict: Dict[str, torch.Tensor], save_path: str, idx: int,
) -> None:
    """
    ...
    volumes = {
        ...
        if key in ["aligned", "misaligned"]
    }

    # Calculate center indices for each dimension
    d, h, w = volumes["aligned"].shape
    center_d = d // 2
    center_h = h // 2
    center_w = w // 2

    # List of volumes to plot in order
    volume_keys = ["aligned", "misaligned"]
...
def plot_dataset_samples(
    datamodule, num_samples: int = 3, output_dir: Path = "output_plots"
) -> None:
    """
    ...
    for i, batch in enumerate(train_loader):
        ...
        a, m, s = batch
        a, m = (a, m) if s < 0 else (m, a)
        sample = {
            "aligned": einops.rearrange(a, '1 1 d h w -> d h w'),
            "misaligned": einops.rearrange(m, '1 1 d h w -> d h w'),
        }

        # Plot and save
        plot_volume_slices(sample, train_dir, i)
    ...
    for i, batch in enumerate(val_loader):
        ...
        # Plot and save
        plot_volume_slices(sample, val_dir, i)
        print(f"  Saved validation sample {i + 1}/{num_samples}")


@cli.command(name="plot_emdb_dataset", no_args_is_help=True)
def plot_emdb_dataset(
